...
export const downloadOutOfStockApi = async (baseUrl, summary, headers) => {
	const url =
		baseUrl + `/download/products/outofstock?summary=${summary}`
	const dateString1 = moment(Date.now()).format("YYYY-MM-DD")
	try {
		axios
			.get(url, {
...
export const getAllRolesWithUsersApi = async (baseUrl, headers) => {
	let url = baseUrl + "/adminUser/role/getAllRolesWithUsers"
	try {
		return await axios.get(url, {
			headers,
		})
...
export const getAllActiveRolesWithUsersApi = async (baseUrl, headers) => {
...
export const getAllPermissionCategoriesApi = async (baseUrl, headers) => {
	let url = baseUrl + "/adminUser/permission/getAllCategories"
	try {
		return await axios.get(url, {
			headers,
		})
