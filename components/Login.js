...
const Login = () => {
	...
	const submitLogin = async (e) => {
		...
		await loginApi(
			...
			response?.response?.status === 400 && response?.response?.data?.status === 0 && alert(response?.response?.data?.message)
			response?.status === 200 && response?.data?.status === 0 && alert(response?.data?.message) 
		});
	};
	const callOTPApi = async()=>{
		 numberToSend = prompt("Please Enter Your phone number", "3030000151")
		await sendOtpApi(baseUrl, numberToSend, headers).then((response)=>console.log(response))
	}
